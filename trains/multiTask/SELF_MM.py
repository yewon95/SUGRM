...
class SELF_MM():
    def __init__(self, args):
        ...
        self.feature_map = {
            ...
            'text': torch.zeros(args.train_samples, args.common_dim, requires_grad=False).to(args.device),
            'audio': torch.zeros(args.train_samples, args.common_dim, requires_grad=False).to(args.device),
            'vision': torch.zeros(args.train_samples, args.common_dim, requires_grad=False).to(args.device),
        }
        ...
        self.dim_map = {
            'fusion': torch.tensor(args.common_dim).float(),
            'text': torch.tensor(args.common_dim).float(),
            'audio': torch.tensor(args.common_dim).float(),
            'vision': torch.tensor(args.common_dim).float(),
        }
        # new labels
        self.label_map = {
        ...
        self.max_min_feat = {
            'feat_with_label_p3': torch.tensor(args.common_dim).float(),
            'feat_with_label_n3': torch.tensor(args.common_dim).float()
        }
    ...
    def do_train(self, model, dataloader):
        ...
        while True: 
            ...
            if self.epochs - best_epoch >= self.args.early_stop:
                ...
                M_zip = ", ".join(map(str, list(zip(torch.cat(y_true["M"]), torch.cat(y_pred['M'])))))
                T_zip = ", ".join(map(str, list(zip(torch.cat(y_true['T']), torch.cat(y_pred['T'])))))
                A_zip = ", ".join(map(str, list(zip(torch.cat(y_true['A']), torch.cat(y_pred['A'])))))
                V_zip = ", ".join(map(str, list(zip(torch.cat(y_true['V']), torch.cat(y_pred['V'])))))
                raw_text_str = ", ".join(map(str, raw_text_list))
                logger.info("Raw text: >> " + raw_text_str)
                logger.info('M_true/pred: >> ' + M_zip)
                logger.info('T_true/pred: >> ' + T_zip)
                logger.info('A_true/pred: >> ' + A_zip)
                logger.info('V_true/pred: >> ' + V_zip)
                return
